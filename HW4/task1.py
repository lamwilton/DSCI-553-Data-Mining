...
def graph_construct():
    ...
    Users count (after filtering) = 3374
    Edges count = 498
    ...
    candidate_pairs = all_pairs.filter(lambda x: corated_helper(user_reviews_dict, x[0], x[1])) \
        .collect()
    return users, candidate_pairs, users_dict, users_inv
...
if __name__ == '__main__':
    ...
    # ========================================== Graph Construction ==========================================
    users, candidate_pairs, users_dict, users_inv = graph_construct()
    row = Row("id")
    vertices = sc.parallelize(users).map(row).toDF()
    edges = sc.parallelize(candidate_pairs).toDF(["s", "t"])
